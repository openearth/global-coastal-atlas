...
if __name__ == "__main__":
    ...
    dataset_dir = gca_data_dir.joinpath(r"01_Hazards\02_Future\Extreme_Wave_Height\GCM_Projection_2090-2100")
    cred_dir = pathlib.Path(p_drive, "11207608-coclico", "FASTTRACK_DATA")
    IN_FILENAME = "EC-EARTH_WW3_3Hourly_2090-2100.zarr"  # original filename as on P drive
    OUT_FILENAME = "Haz-GCM_90-100.zarr"  # file name in the cloud and on MapBox
    VARIABLES = [
        "hs_1_26",
        "hs_5_85"
    ...
    date_range = pd.date_range(start='2040-01-01', end='2050-12-31', freq='AS')
    MAP_SELECTION_DIMS = {
        "time": np.array([date.year for date in date_range])  # TODO: fix bug. If I put ["ssp126", "ssp245", "ssp585"] if doesn't work properly as nscenarios and scenarios are disconnected in the variable. If I put nscenarios = [0,1,2] (like for ensembles) I get an error because of the disconnection
    ...
    DIMENSIONS_TO_IGNORE = ["lat","lon","stations"]  # dimensions to ignore
    ...
    years = np.array([date.year for date in date_range])
    ds=ds.sel(time=date_range,method="nearest")
    ds["time"]=np.array([date.year for date in date_range])

    # ds = ds.drop_dims(['dimension1', 'dimension2'])
    ...
    print("starting collection_creation")
    for var in VARIABLES:
        collection = get_geojson(
            ds.drop_dims(['lon', 'lat'], errors='ignore'),
            variable=var,
            dimension_combinations=dimcombs,
            # dimension_combinations=pd.to_datetime(dimcombs, unit='ns'),
