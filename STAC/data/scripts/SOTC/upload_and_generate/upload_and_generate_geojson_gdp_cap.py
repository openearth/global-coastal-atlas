...
if __name__ == "__main__":
    ...
    dataset_dir = gca_data_dir.joinpath(r"03_Vulnerability\GDP")
    cred_dir = pathlib.Path(p_drive, "11207608-coclico", "FASTTRACK_DATA")
    IN_FILENAME = "GDP_PPP_CAP_final.zarr"  # original filename as on P drive
    OUT_FILENAME = "Vul-GDP_Cap.zarr"  # file name in the cloud and on MapBox
    VARIABLES = [
        # "GDP_PPP_Cap",
        "gdp_ppp_cap_3_yr"
    ...
    MAP_SELECTION_DIMS = {
        ...
        # "time": pd.date_range(start='1960-01-01T00:00:00.000000000', end='2022-01-01T00:00:00.000000000', freq='YS').to_numpy()
    }
    ...
    lat=ds.lat.fillna(0),
    lon=ds.lon.fillna(0),
    gdp_ppp_cap_3_yr=ds.gdp_ppp_cap_3_yr.fillna(0),
    )
    ds=ds.rename_vars({"gdp_ppp_cap_3_yr":"GDP_3yr"})
    VARIABLES = ["GDP_3yr"]
    ...
    for var in VARIABLES:
        # ds=ds.fillna(value=None)  #clenmar added
        ...
        with dataset_dir.joinpath("platform") as outdir:
            ...
            # def replace_nan_with_none(feature):
            #     from math import isnan
            #     """Recursively replace NaN values with None in a GeoJSON feature."""
            #     if isinstance(feature, dict):
            #         for key, value in feature.items():
            #             feature[key] = replace_nan_with_none(value)
            #     elif isinstance(feature, list):
            #         for i, item in enumerate(feature):
            #             feature[i] = replace_nan_with_none(item)
            #     elif isinstance(feature, float) and isnan(feature):
            #         return None  # Replace NaN with None
            #     else:
            #         return feature

            # # Preprocess the collection
            # collection = replace_nan_with_none(collection)
