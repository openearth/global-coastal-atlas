...
if __name__ == "__main__":
    ...
    dataset_dir = gca_data_dir.joinpath(r"01_Hazards\01_Present\Extreme_Tropical_Cyclone_Wind\PC_1000yrs_cat_3_4")
    cred_dir = pathlib.Path(p_drive, "11207608-coclico", "FASTTRACK_DATA")
    IN_FILENAME = "Extreme_Tropical_Cyclone_Wind_final.zarr"  # original filename as on P drive
    OUT_FILENAME = "Haz-TC_CAT3_RP.zarr"  # file name in the cloud and on MapBox
    VARIABLES = [
        "TC_RP_map_96kts_PC",
        "TC_RP_map_96kts_FC_2100_RCP45",
        "TC_RP_map_96kts_FC_2050_RCP85"

    ]  # what variable(s) do you want to show as marker color?

    ###############################
    #   OUT_FILENAME (NO_ZARR) + VARIABLES <32
    ################################
    MUCH_SHORTER_OUT_FILENAME= "C.zarr"  # file name in the cloud and on MapBox
    ...
    ds_flat = ds.drop_dims(['latitude', 'longitude'])

    #reshape

    lon_vals=np.tile(ds["longitude"].values,len(ds["latitude"].values))
    lat_vals=np.tile(ds["latitude"].values,len(ds["longitude"].values))

    list(ds.data_vars.keys())
    for name in list(ds.data_vars.keys()):

        print(ds[name].values[0].shape)
        array=ds[name].values[0]
        reshaped_array = array.reshape(1, len(ds["latitude"].values) * len(ds["longitude"].values))[0]
        reshaped_array
        ds_flat[name]=("nstations",reshaped_array)

    ds_flat["lon"]=("nstations",lon_vals)
    ds_flat["lat"]=("nstations",lat_vals)
    ds=ds_flat
    ...
    for var in VARIABLES:
        ...
        OUT_FILENAME=MUCH_SHORTER_OUT_FILENAME
