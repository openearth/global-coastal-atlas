...
if __name__ == "__main__":
    ...
    gca_data_dir = pathlib.Path(
        ...
        p_drive,
        "11209197-018-global-coastal-atlas",
        "MSc_students\ClenmarRowe\Data\All_Datasets\Orig_Datasets",
        r"01_Hazards\02_Future\Extreme_Wave_Height",
    )
    dataset_dir = gca_data_dir.joinpath("From Alberto_updated_fixing Low waves near NL issue")

    # opening metadata
    metadata_fp = dataset_dir.joinpath("metadata_Meucci_21st_century_Wave_heights.json")
    ...
    DATASET_FILENAME = "Haz-Hs_Meucci_2100.zarr"
    VARIABLES = ["Meucci_RP100_percent_change_end21C_RCP45", "Meucci_RP100_percent_change_end21C_RCP85", "atGTSR_POT_EXP_RP100_ZtoHs_ens1000_CMIP5_ww3_hs_19792005"]  # xarray variables in dataset
    ...
    #####
    COLLECTION_ID = DATASET_FILENAME[0:-5]
    #####
    ...
    UNITS = ""
    PLOT_SERIES = "Meucci_RP100_percent_change_end21C_RCP85"
    PLOT_X_AXIS = "nstations"
    PLOT_TYPE = "line"
    MIN = 0
    MAX = 3
    LINEAR_GRADIENT = [
    ...
    catalog = Catalog.from_file(
        os.path.join(pathlib.Path(__file__).parent.parent.parent.parent, STAC_DIR, "catalog.json")
    )
    ...
    collection.normalize_hrefs(
        os.path.join(pathlib.Path(__file__).parent.parent.parent.parent, STAC_DIR, COLLECTION_ID),
        strategy=layout,
    )

    catalog.save(
        catalog_type=CatalogType.SELF_CONTAINED,
        dest_href=os.path.join(pathlib.Path(__file__).parent.parent.parent.parent, STAC_DIR),
