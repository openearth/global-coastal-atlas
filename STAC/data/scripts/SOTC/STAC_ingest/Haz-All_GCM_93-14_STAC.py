...
if __name__ == "__main__":
    ...
    BUCKET_PROJ = "gca/SOTC/Haz-Offshore_Waves/Present_Climate/GCM_Hindcasts_CMIP6"
    ...
    GCM_all=["MRI-ESM2-0","MPI-ESM1-2-LR","KIOST-ESM","IPSL-CM6A-LR","CMCC-CM2-SR5","AWI-CM-1-1-MR","EC-EARTH3","ACCESS-CM2"]

    for i,GCM in tqdm(enumerate(GCM_all),desc="GCM completed"):# Project paths & files (manual input)

        # hard-coded input params at project level

        dataset_dir = gca_data_dir.joinpath(r"01_Hazards\02_Future\Extreme_Wave_Height\GCM_All_8_CMIP6_Hindcast_1993-2014")

        # opening metadata
        metadata_fp = dataset_dir.joinpath(f"metadata_{GCM}_Hindcast-1993-2014.json")
        ...
        DATASET_FILENAME = f"Haz-{GCM}_Hind_93-14.zarr"
        VARIABLES = [f"hs_{GCM}"]  # xarray variables in dataset
        ...
        UNITS = "m"
        PLOT_SERIES = f"hs_{GCM}"
        PLOT_X_AXIS = "latitude"
        PLOT_TYPE = "line"
        MIN = 0
        MAX = 3
        LINEAR_GRADIENT = [
